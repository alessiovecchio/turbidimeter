#!/usr/bin/python
# -*- coding:utf-8 -*-
#----------WE ARE NOT THE OWNERS OF THIS CODE, WE SHOW IT BECAUSE WE MODIFIED IT,
#----------PLEASE DOWNLOAD THE LIBRARY FROM THE OFFICIAL SITE:
#http://www.cqrobot.wiki/index.php/TSL25911FN_Ambient_Light_Sensor_SKU:_CQRTSL25911
...
class TSL2591:
    def __init__(self, busNumber, address=ADDR):
        self.i2c = smbus.SMBus(busNumber)
        self.address = address
        
        GPIO.setmode(GPIO.BCM)
        GPIO.setwarnings(False)
        if(busNumber == 0):
            GPIO.setup(INI_PIN1, GPIO.IN)
        else:
            GPIO.setup(INI_PIN3, GPIO.IN)
    ...
    def Lux(self):
        ...
        for i in range(0, self.IntegralTime+2):
            time.sleep(0.1)
        #guardarlo meglio e modificarlo per i due casi diversi, gestirli separatamente
        if(GPIO.input(INI_PIN1) == GPIO.HIGH and GPIO.input(INI_PIN3) == GPIO.HIGH):
            print ('INT 0')
        else:
            print ('INT 1')
