<?php
	...
	if(isIntervalValid($beginningDate, $endDate)){ //Controllo se l'intervallo è valido
		$result = getVoltageData($turbidimeterId, $beginningDateFormatted->format('Y-m-d H:i:s'), $endDateFormatted->format('Y-m-d H:i:s'));
	}else{
	...
	$message = "OK";	
	$response = setVoltageResponse($result, $message);
	echo json_encode($response);
	return;
	...
	function setVoltageResponse($result, $message){
		...
		while ($row = $result->fetch()){
			...
			$sensorData->voltage = $row['voltage'];
